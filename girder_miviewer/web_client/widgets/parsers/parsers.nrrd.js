/** * Imports ***/
import ParsersVolume from 'ami.js/src/parsers/parsers.volume.js';
...
export default class ParsersNifti extends ParsersVolume {
  ...
  constructor(data, id) {
    ...
    try {
      ...
      this._reconstructNrrdHeader = this._reconstructNrrdHeader();
    } catch (error) {
  ...
  /**
   * Reconstruct data header from parser for creating segmentation
   *
   * @return {*}
   */
  _reconstructNrrdHeader(){
    let header = 'NRRD0001\n\
# Complete NRRD file format specification at:\n\
# http://teem.sourceforge.net/nrrd/format.html\n';
    if (this._dataSet.type === 'int8' ||
       this._dataSet.type === 'char') {
      header = header.concat('type: signed char\n')
    } else if (this._dataSet.type === 'uint8') {
      header = header.concat('type: unsigned char\n')
    } else if (this._dataSet.type === 'int16' ||
       this._dataSet.type === 'short') {
      header = header.concat('')
    } else if (this._dataSet.type === 'uint16') {
      header = header.concat('type: unsigned short\n')
    } else if (this._dataSet.type === 'int32') {
      header = header.concat('')
    } else if (this._dataSet.type === 'uint32') {
      header = header.concat('')
    ...
    header = header.concat('dimension: ' + this._dataSet.dimension + '\n');
    header = header.concat('space: ' + this._dataSet.space + '\n');
    header = header.concat('sizes: ' + this._dataSet.sizes[0] + ' '
                            + this._dataSet.sizes[1] + ' '
                            + this._dataSet.sizes[2] + '\n');
    header = header.concat('space directions: (' + this._dataSet.spaceDirections[0][0] + ','
                                        + this._dataSet.spaceDirections[0][1] + ','
                                        + this._dataSet.spaceDirections[0][2] + ') ('
                                        + this._dataSet.spaceDirections[1][0] + ','
                                        + this._dataSet.spaceDirections[1][1] + ','
                                        ...
                                        + this._dataSet.spaceDirections[2][1] + ','
                                        + this._dataSet.spaceDirections[2][2] + ')' + '\n');
    header = header.concat('kinds: ' + this._dataSet.kinds[0] + ' '
                            + this._dataSet.kinds[1] + ' '
                            + this._dataSet.kinds[2] + '\n');

    if(this._dataSet.endian !== 'undefined'){
      header = header.concat('endian: ' + this._dataSet.endian + '\n');
    }

    header = header.concat('encoding: ' + this._dataSet.encoding + '\n');
    header = header.concat('space origin: (' + this._dataSet.spaceOrigin[0] + ','
                                    + this._dataSet.spaceOrigin[1] + ','
                                    + this._dataSet.spaceOrigin[2] + ')\n\n');
    return header;
  }
