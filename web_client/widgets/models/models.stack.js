...
import CoreColors from 'ami.js/src/core/core.colors.js';
import CoreUtils from 'ami.js/src/core/core.utils.js';
import ModelsBase from 'ami.js/src/models/models.base.js';
...
export default class ModelsStack extends ModelsBase {
  ...
  /**
   * unPack current stack pixel data back to original bits array buffers
   */
  unPack(rawData) {
    const nbVoxels =
    ...
    let voxelIndexStart = 0;
    let voxelIndexStop = nbVoxels;

    let unPacked =
        this._unPackToOriginalBits(
          rawData,
          this._numberOfChannels,
          this._frame,
          nbVoxels,
          voxelIndexStart,
          voxelIndexStop);
      this._oriTextureType = unPacked.textureType;
      this._oriRawData.push(unPacked.data);
  ...
  _unPackToOriginalBits(rawData, channels, frame, nbVoxels, startVoxel, stopVoxel) {
    const unPacked = {
      textureType: null,
      data: null,
    };
    ...
    if (bitsAllocated === 8 && channels === 1 || bitsAllocated === 1) {
      let data = new Uint8Array(nbVoxels);
      for (let i = startVoxel; i < stopVoxel; i++) {
        frameIndex = ~~(i / frameDimension);
        inFrameIndex = i % (frameDimension);


        data[i] = rawData[packIndex];

        packIndex++;
      }
      unPacked.textureType = THREE.LuminanceFormat;
      unPacked.data = data;
    }else if (bitsAllocated === 16 && channels === 1) {
      let data = new Uint16Array(nbVoxels);
      let coordinate = 0;
      let channelOffset = 0;

      for (let i = startVoxel; i < stopVoxel; i++) {
        let index_l = 4 * coordinate + 2 * channelOffset;           //0xFF
        let index_m = 4 * coordinate + 2 * channelOffset + 1;       //0x00
        let least_significant = rawData[index_m];                 //0xFF.    ?????????????
        let most_significant = rawData[index_l];                  //0x00
        data[i] =  ((most_significant & 0x00FF) << 8) | (least_significant & 0x00FF);
      ...
      unPacked.textureType = RGBAFormat;
      unPacked.data = data;
    }
    return unPacked;
  }
  ...
  set oriTextureType(textureType) {
    this._oriTextureType = oriTextureType;
  }

  get oriTextureType() {
    return this.oriTextureType;
  }
  ...
  set oriRawData(rawData) {
    this._oriRawData = oriRawData;
  }

  get oriRawData() {
    return this._oriRawData;
  }
